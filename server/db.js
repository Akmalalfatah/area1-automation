...
export async function loadPreventiveRows(){if(!databaseEnabled)return [];const [records]=await getPool().query('SELECT dataset,upload_date,updated_at FROM preventive_uploads ORDER BY upload_date,updated_at');return records.flatMap(record=>{const dataset=typeof record.dataset==='string'?JSON.parse(record.dataset):record.dataset;return(dataset?.rows||[]).map(row=>({...row,_upload_date:iso(record.upload_date),_updated_at:record.updated_at?.toISOString?.()||record.updated_at}))})}
