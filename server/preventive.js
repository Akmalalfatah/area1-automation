...
const aliases={area:['area'],regional:['regional','region'],nop:['nop','nopname','namanop','nopcluster'],cluster:['clustername','cluster'],ticket_no:['ticketno','ticketnumber','ticket'],site_id:['siteid','idsite','sitecode','sitekode','site'],site_name:['sitename','namasite'],class_site:['classsite','siteclass','kelas'],type_site:['typesite','sitetype','tipe'],type_power:['typepower','jenispower'],scope_item_name:['scopeitemname','scopeitem','scope'],interval:['interval','maintenanceinterval'],last_maintenance:['lastmaintenance','lastmaintenancedate','maintenancebefore'],schedule_date:['scheduledate','planscheduledate','plandate','schedule','tanggalplan','jadwal'],diff_days:['diffdays','daydifference','selisihhari'],status:['status','pmstatus','maintenancestatus'],submitted_date:['submitteddate','submitdate','submissiondate','tanggalsubmit','submitted'],pic:['pic','personincharge'],notes:['notes','note','catatan','remark','remarks','keterangan'],created_date:['createddate','creationdate','tanggalbuat']}
const normalize=value=>String(value??'').trim().toLowerCase().replace(/[^a-z0-9]/g,'')
const fieldFor=value=>Object.entries(aliases).find(([,values])=>values.includes(normalize(value)))?.[0]
const pad=n=>String(n).padStart(2,'0')
const maintenanceKindFromFilename=filename=>{const name=String(filename||'').toLowerCase();if(name.includes('genset'))return'genset';if(name.includes('punchlist'))return'punchlist';return'site'}
...
export async function parsePreventiveWorkbook(buffer,filename){
  ...
  const maintenance_kind=maintenanceKindFromFilename(filename),rows=[]
  for(let row=headerRow+1;row<=selected.rowCount;row++){const record={};for(const [col,field] of Object.entries(headerMap))record[field]=selected.getCell(row,Number(col)).value;const site=String(record.site_id??'').trim(),schedule=isoDate(record.schedule_date);if(!site||!schedule)continue;const submitted=isoDate(record.submitted_date);rows.push({maintenance_kind,site_id:site,site_name:String(record.site_name||'-').trim()||'-',nop:String(record.nop||'Belum ditentukan').trim()||'Belum ditentukan',area:String(record.area||'').trim(),regional:String(record.regional||'').trim(),cluster:String(record.cluster||'').trim(),ticket_no:String(record.ticket_no||'').trim(),class_site:String(record.class_site||'').trim(),type_site:String(record.type_site||'').trim(),type_power:String(record.type_power||'').trim(),scope_item_name:String(record.scope_item_name||'').trim(),interval:String(record.interval||'').trim(),last_maintenance:submitted,schedule_date:schedule,diff_days:record.diff_days==null?'':String(record.diff_days).trim(),status:String(record.status||'').trim(),submitted_date:submitted,pic:String(record.pic||'').trim(),notes:String(record.notes||'').trim(),created_date:isoDate(record.created_date)})}
  ...
  return{filename,sheet:selected.name,maintenance_kind,rows,row_count:rows.length,date_start:rows.map(x=>x.schedule_date).sort()[0],date_end:rows.map(x=>x.schedule_date).sort().at(-1),data_month:dataMonth}
}
