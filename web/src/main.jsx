...
function PreventiveDashboard({data,file,date,busy,loading,filters,onFile,onDate,onUpload,onFilter}){
  ...
  return <div className="space-y-4">
    ...
    <div className="grid grid-cols-4 gap-4"><PreventiveSummaryCard label="PLAN SITE" value={data?.plan??0} detail={`${shortDate(filters.dateFrom)} sampai ${shortDate(filters.dateTo)}`} tone="navy"/><PreventiveSummaryCard label="ACTUAL SUBMITTED" value={data?.actual??0} detail="Site unik dengan Actual Date terisi" tone="green"/><PreventiveSummaryCard label="ACHIEVEMENT" value={`${formatNumber(data?.achievement??0)}%`} detail="Actual dibanding Plan pada filter aktif" tone="orange"/><PreventiveSummaryCard label="BELUM SUBMIT" value={data?.pending??0} detail="Site plan yang belum memiliki Actual Date" tone="red"/></div>
